...
- 您在应用上花费的总时间
- 您设备使用的操作系统
- 设备唯一标识符（如设备ID），用于请求限流和防止服务滥用，以确保应用的稳定性和安全性（例如，避免请求过载、保障服务可用性）。
...
💡 重要说明：设备唯一标识符（如设备ID）仅用于内部请求限流（例如，防止同一设备频繁请求导致服务拥堵），不会与任何第三方共享，且不会关联您的个人身份信息。该标识符在服务器端会进行匿名化处理，无法用于追踪您的个人行为
...
⚠️ 特别说明：设备唯一标识符仅用于内部服务优化，绝不与任何第三方共享（包括广告商、数据分析平台等）。所有设备ID数据均在服务端匿名化处理，不包含任何可识别个人身份的信息。
---
